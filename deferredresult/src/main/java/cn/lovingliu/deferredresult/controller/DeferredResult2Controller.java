...
/**
 * @Author：LovingLiu
 * @Description: DeferredResult 实现非阻塞式调用 （仅仅生成DeferResult）
 * @Date：Created in 2019-12-19
 */
@RestController
public class DeferredResult2Controller {
    private Logger log = LoggerFactory.getLogger(DeferredResult2Controller.class);
    ...
    public DeferredResult<ResponseMsg<String>> getResult(){
        ...
        result.onTimeout(() -> {
            log.info("调用【超时】，移除任务，此时队列长度为{}",taskSet.getSet().size());
        ...
        result.onCompletion(() -> {
            log.info("调用【完成】，移除任务，此时队列长度为{}",taskSet.getSet().size());
